...
//    NSLog(@"jsCode %@",jsCode);
    ...
    float actualHeight=height.integerValue;
    float actualWidth=width.integerValue;
    ...
    if ([[UIDevice currentDevice] userInterfaceIdiom] ==UIUserInterfaceIdiomPhone) {
       ...
       // CGFloat screenHeight = screenRect.size.height;
        ...
        actualHeight=actualHeight/actualWidth;
        actualHeight=actualHeight*screenWidth;
        actualHeight=actualHeight-10;
        actualWidth=screenWidth-10;
       // [self.view bringSubviewToFront:_webView];
    }else{
    NSInteger idealWidth=768;
    NSInteger idealHeight=984;
        
    if (height.integerValue>idealWidth&&width.integerValue>idealWidth) {
       actualHeight= height.floatValue/width.floatValue;
        actualHeight=actualHeight*idealWidth;
    ...
    }else if(height.integerValue>idealHeight){
        actualHeight=idealHeight;
        actualWidth=width.floatValue/height.floatValue;
        actualWidth=actualWidth*idealHeight;
    ...
    }else if(width.integerValue>idealWidth){
        actualWidth=idealWidth;
        actualHeight= height.floatValue/width.floatValue;
        actualHeight=actualHeight*idealWidth;
    }
...
- (IBAction)showOrHideButton:(id)sender {
    if (_hide) {// if hidden
        [UIView animateWithDuration:1.0 animations:^{
            ...
            frame=_hideOrShow.frame;
            frame.origin.y=_topView.frame.size.height;
            _hideOrShow.frame=frame;
...
//    UIView *blackView = [[UIView alloc] initWithFrame:CGRectMake(463, -4, 300, 44)];
//    [blackView setBackgroundColor:[UIColor blackColor]];
    
//    _searchBar.frame=blackView.bounds;
 
//    [_searchBar removeFromSuperview];
//    [self.topView addSubview:blackView];
//    [blackView addSubview:_searchBar];
//ViewController *controller=(ViewController *)self.parentViewController.parentViewController;
//
//[controller loadSpine:_chapter.chapterIndex+1 atPageIndex:_chapter.chapterIndex+1 highlightSearchResult:nil];
}
...
//        UIMenuItem *itemNotes=[[UIMenuItem alloc]initWithTitle:@"Notes" action:@selector(notes:)];
//        
//        NSArray *array=@[itemHightlight,removeHighlight,itemNotes];
//        menuController.menuItems=array;
//        [itemHightlight release];
//        [itemNotes release];
//        [removeHighlight release];
...
/*- (void)dealloc {
    ...
    [_showNotesOrHighlight release];
    [_highlightAsNotes release];
    [_hideTouchUpInsider release];
    [_tocButton release];
    [_searchBar release];
    [super dealloc];
}*/
...
//    [p presentPopoverFromRect:button.frame inView:button permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
