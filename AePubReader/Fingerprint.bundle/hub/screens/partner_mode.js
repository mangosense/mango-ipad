...
o = function(s, args) {
    ...
    var partners = ["fingerprint", "astro"];
    var features = ["bHasKidsOrFamily (premium subscriber)"];
    var settingName = ["PartnerMode_bHasKidsOrFamily"];
    ...
    function init()
    {
        ...
        label({parent:p, id: "title", string: "Partner Mode", center: true, vCenter:true, x: 80, y: 0, w: 160, h: 40, size:12.5, color:"#4e4e4e"});
        button({parent: p, src:gImagePath+"gray-leftpoint-button", idleover:"same", x: 10, h: 30, w: 60, y: 5, size: 12, id: "back", string: i18n('_BACK')});

        var oy = 50;
        label({parent:p, x: 10, y: oy, w: 200, h: 30, size: 18, string: "Partner Value", color:"#4e4e4e"});
        oy += 30;
        for (var i=0; i<partners.length; i++) {
            var b = button(_CheckBoxButton, {parent:p, id: "partner."+i, x: 10, y:oy, w:25, h:25});
            label({parent:p, x: 40, y: oy+5, w: 250, h: 30, size: 15, string: partners[i], color:"#4e4e4e"});
            oy += 30;
            SetToggle(b, FPGetAppValue("partner") === partners[i]);
        }

        oy += 30;
        label({parent:p, x: 10, y: oy, w: 200, h: 30, size: 18, string: "Features", color:"#4e4e4e"});
        oy += 30;
        for (var i=0; i<features.length; i++) {
            var b = button(_CheckBoxButton, {parent:p, id: "feature."+i, x: 10, y:oy, w:25, h:25});
            label({parent:p, x: 40, y: oy+5, w: 250, h: 30, size: 15, string: features[i], color:"#4e4e4e"});
            oy += 30;
            SetToggle(b, FPGetAppValue(settingName[i]));
        }
    }
    ...
    p.on_partner = function(j)
    {
        var partner = partners[j];
        for (var i=0; i<partners.length; i++) {
            SetToggle(s.button["partner."+i], i == j);
        }
        FPSetAppValue("partner", partner);
    }

    p.on_feature = function(i)
    {
        var bOn = s.button["feature."+i].bOn;
        FPSetAppValue(settingName[i], bOn);
    }
