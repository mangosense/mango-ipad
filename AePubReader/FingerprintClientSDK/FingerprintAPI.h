...
#ifndef _FINGERPRINT_API_H_
#define _FINGERPRINT_API_H_

/*! \mainpage

 <h2>Welcome to the iOS Fingerprint SDK C++ API</h2>
...
public:
	/**
     ...
     Each time the onLoginComplete callback is received, the game should reload the user data using the FingerprintAPI::loadData call.
	 */
	virtual void			onLoginComplete() = 0;
	...
	/**
     ...
     In your onUIComplete handler, please call FingerprintAPI::avatarDescription to obtain the current language and see if it has changed.
	 ...
	 */
	virtual void			onUIComplete(const char* mode) = 0;

	/** 
	 after a call to FingerprintAPI::upload, onUploadComplete is called when an upload has completed
	 
	 @param uploadID the id that was retuned by the call to FingerprintAPI::upload
	 ...
	 */
	virtual void			onUploadComplete(int uploadID, bool bSuccess, const char* url) = 0;

	/**
	 called when a data state conflict arises
	 
	 When calling FingerprintAPI::loadData, it's possible for a conflict to arisen if another iOS device uploaded
	 ...
	 @param dataID the data identifier is being loaded - same as the dataID that was passed to FingerprintAPI::loadData
	 ...
	 @param result the place to put the reconciled result - note it is pre-populated with the "server" value - as such,
	 the default behavior, if you supply only an empty implementation is to use the newer data from the server
	 */
	virtual void			onResolve(const char* dataID, const JSONNode& server, const JSONNode& client, JSONNode& result) = 0;
     ...
     */
    virtual void            onSendMessageResponse(const JSONNode& data) = 0;
     ...
     */
    virtual void            onGameUpdate(const JSONNode& game) = 0;
    ...
    /**
     ...
     between a call to loading true and loading false. This can significantly speed the loading on low-end
     ...
     @param bLoading true if loading starting, false if loading completed.
     */
    virtual void            onLoading(bool bLoading) = 0;
     ...
     */
    virtual void            onParentGate(bool bIsParent, const JSONNode& details) = 0;
};
...
public:
	/**
	 ...
	 @param launchOptionsNSDictionary is the launchOptions passed to didFinishLaunchingWithOptions by iOS
     @param fpOptions specifies the Fingerprint options settings for your app. Options include the following:\n\n
     Name: "bMultiplayer"\n
     Possible Values: 0 or 1\n
     Description: If the app is a multiplayer game, you must set "bMultiplayer" to 1 in fpOptions. If "bMultiplayer" is not provided or is 0, the Fingerprint platform treats the app as a single player game.\n\n
     Name: "bLandscape"\n
     Possible Values: 0 or 1\n
     Description: If the app is a landscape game, you must set "bLandscape" to 1 in fpOptions. If "bLandscape" is not provided or is 0, the app is treated as a portrait game.\n\n
     Name: "bPracticeRound"\n
     Possible Values: 0 or 1\n
     Description: If the app supports a practice round, set to 1. If "bPracticeRound" is not provided or is 0, the platform will not allow a practice round.\n\n
     ...
     pause screen upon resume. If "bSuppressPauseScreen" is 1, the user will be taken to the game (which may provide its own pause screen). If "bSuppressPauseScreen"
     is not provided or is 0, the Fingerprint platform will present its own pause screen.\n\n
	 @param delegate is a pointer to the FingerprintAPIDelegate for your app
	 ...
	 */
	static bool             Startup(void* launchOptionsNSDictionary, JSONNode& fpOptions, FingerprintAPIDelegate* delegate);
	
	/**
	 Call to obtain the singleton instance of the FingerprintAPI object.
	 Only valid after calling Startup.
	 
	 */
	static FingerprintAPI*	GetAPI();
     ...
     */
    void                    openURL(void* url, void* sourceApplication, void* annotation);
	...
	/**
	 ...
	 FingerprintAPIDelegate::onLoginComplete will get called when everything is ready to go
	 */
	void					login();
	 ...
	 */
	void					openHub(const char* mode, const JSONNode& data);
	...
	/**
	 ...
	 FingerprintAPIDelegate::onUIComplete will be called.

	 */							
	void					sendMessage(const char* templateID, const JSONNode& data, bool bInteractive);
	 ...
	 */
	void					metric(const char* metricID, const JSONNode& data);
	 ...
	 */
	void					metricScreen(const char* screenID);
	 ...
	 */
	void					reportProgress(const char* progressID, const JSONNode& data);
	...
	/**
     ...
     JSONNode will contain the following name/value pairs:\n\n
     Name: "hair"\n
     ...
     @param result JSONNode to receive the result
     */
	void					avatarDescription(JSONNode& result);
	 ...
	 */
	void					saveData(const char* dataID, const JSONNode& data);
	...
	/**
     ...
     @param dataID name of data to save
	 */
	void					deleteData(const char* dataID);
	
	/**
	 Load persisted data. 
	 
	 Note that if a data conflict occurs, FingerprintAPIDelegate::onResolve will be called.
	 
	 This does not trigger any User Interface.

     @param dataID name of data to save
     @param result object to receive the JSON object 
     */
	void					loadData(const char* dataID, JSONNode& result); 


	/**
	 Upload data to create an file hosted by Fingerprint on the Internet
	 
	 When the upload is complete, FingerprintAPIDelegate::onUploadComplete will be called with the returned uploadID,
	 whether or not the upload was successful, and if it was, the URL where the data can now be found on the Internet
	 
	 @param fileSuffix the filename on the Internet will be a unique string, but you can supply the file suffix to help in identifying the format of the data when it is downloaded later.
     @param buffer the data to upload
     @param length the length of the data to upload
	 
	 @return returns an integer id to identify the completion event to FingerprintAPIDelegate::onUploadComplete
	 */
	int						upload(const char* fileSuffix, const char* buffer, unsigned long length);
    ...
    /**
     ...
     @param details for future expansion
     */
    void                    parentGate(const JSONNode& details);
     ...
     */
    void					sendMove(const JSONNode& move);
     ...
     */
    void						storePushNotificationToken(void* deviceToken);
    
    /**
     Call when your App receives for a push notification initiated by Fingerprint:
     - (void)application:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)userInfo
     
     This call will adjust the number set as the badge of the application icon if necessary.
     
     @param userInfo is the userInfo passed to application:didReceiveRemoteNotification by iOS
     */
    void						receivedRemoteNotification(void* userInfo);
};


#endif // _FINGERPRINT_API_H_
